...
import re
from SESpider.tools.crawl_ip_tool import crawl_ip_tool

ip_tool = crawl_ip_tool()

# class SespiderSpiderMiddleware(object):
...
class ProcessAllExceptionMiddleware(object):
    ...
    def process_response(self, request, response, spider):
        ...
        if str(response.status).startswith('4') or str(response.status).startswith('5'):
            ...
            print('error code' + request.meta['proxy'])
            return response
        # 其他状态码不处理
        return response
    ...
    def process_exception(self, request, exception, spider):
        ...
        if isinstance(exception, self.ALL_EXCEPTIONS):
            if isinstance(exception,(TimeoutError,ConnectionRefusedError)):
                ...
                print('timeout'+request.meta['proxy'])
...
# class SESpiderDownloaderMiddleware(object):
#     # Not all methods need to be defined. If a method is not defined,
